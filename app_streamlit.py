...
UPLOAD_WEBHOOK = "https://dsc2302023.app.n8n.cloud/webhook/upload-report"

INSIGHT_WEBHOOK = "https://dsc2302023.app.n8n.cloud/webhook/generate-insight"

QUERY_WEBHOOK = "https://dsc2302023.app.n8n.cloud/webhook/query"
...
if page == "Upload Data":
    ...
    if uploaded_file is not None:
        ...
        if st.button("Process and Insert Data"):
                ...
                if result["process"]["status"] == "success":
                    ...
                    with st.container(border=True):
                        st.success(
                            ...
                            "Please navigate to the Prediction page and Analytics Report page to view the prediction results and analytics."
                        )

            # Warning returned from API
            elif result.get("status") == "warning":
                st.warning(result.get("message", "Unknown warning."))
            ...
            # Backend unavailable / unexpected response
            else:
                st.error("Backend service is temporarily unavailable or returned an unexpected response.")
                st.json(result)
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if "process_result" not in st.session_state:
        ...
        high_risk_df = pd.DataFrame(prediction["data"])
    ...
    for chat in reversed(st.session_state.chat_history):
        ...
        with st.chat_message("assistant"):
            ...
            if result["status"] != "success":
                ...
                if visualization is not None and visualization["status"] == "chart":
